...
def send_sns_alert(aws_access_key_id, aws_secret_access_key, region_name, total_in_rupees, start_date, end_date):
    if SNS_TOPIC_ARN == "YOUR_SNS_TOPIC_ARN_HERE": return False
    try:
        ...
        subject = f"🚨 AWS COST ALERT: ₹{total_in_rupees:,.2f} Exceeded Threshold!"
        message = f"Cost Analyzer Report ({start_date} to {end_date}):\nTotal Cost: ₹{total_in_rupees:,.2f}"
...
class PDFReport(FPDF):
    def header(self):
        ...
        self.cell(0, 10, 'AWS Cost Analyzer (INR)', 0, 1, 'C')
        self.ln(10)
    def footer(self):
...
def fetch_cost_data(key, secret, region, start, end):
    ...
    try:
        ...
        sent = send_sns_alert(key, secret, region, tot_inr, start, end) if alert else False
...
# --- HTML TEMPLATE (MINT GREEN THEME) ---
HTML_TEMPLATE = """
<!doctype html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .total-display { font-size: 3rem; font-weight: 800; color: #059669; text-align: center; margin: 10px 0; }
